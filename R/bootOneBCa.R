bootOneBCa <- function (x, parameter, null.hyp = NULL, alternative = c("two.sided",
                                                         "less", "greater"), conf.level = 0.95, type = NULL, R = 9999)
{
    proportion <- mean
    test <- !is.null(null.hyp)
    statistic <- parameter
    boot.statistic <- boot(x, function(x, i) statistic(x[i]),
                           R = R)
